...
#pdf("heatmapMedianTMRCASymm.pdf",paper='special', width=12, height = 12)
#matrixaverageTMRCAPop = matrixaverageTMRCAPop[,!colSums(!is.finite(matrixaverageTMRCAPop))]
#h<-heatmap(matrixaverageTMRCAPop, col= colorRampPalette(brewer.pal(8, "Oranges"))(100))
#legend(x="topleft", legend=c(0,  round(mean(matrixaverageTMRCAPop)), round(max(matrixaverageTMRCAPop))), 
#       fill=colorRampPalette(brewer.pal(8, "Oranges"))(3))
#ggsave("heatmapPopsSoFar.pdf", useDingbats=FALSE)
#dev.off()

### other ways to make heatmaps *************************************************
# heat map without dendrogram but ordered by region
setwd("~/switchdrive/NorthCoastPeru_Master/Ychromosome/Rplots")
library(ggplot2)
meltpopTMRCAordered = meltpopTMRCA[order(meltpopTMRCA$region1),] #don't know whether this is even needed
popSorted = popTable[order(popTable$MacroGroup),]
orderpops = popSorted$Population
# color the names of the populations according to region criteria
region<-table(popSorted$MacroGroup)
colorcod = coloursMacroG # or coloursMacroG when withou arctic
colorstring<-c(rep(NA, length(popSorted$MacroGroup)))
for (i in 1:nrow(matrixaverageTMRCAPop)) {
  colorstring[which(popSorted$MacroGroup==labels(region[i]))]<-colorcod[i]
}
gg<-ggplot(data = meltpopTMRCAordered, aes(x=pop1, y=pop2, fill=averageTMRCA)) + 
  geom_tile()+
  scale_fill_gradient(low = "white",   high = "darkorange")+
  theme(axis.text.x = element_text(angle = 45, vjust=1, hjust=1, colour = colorstring, size = 2.7),
        axis.text.y = element_text(colour = colorstring, size= 2.7))+
  scale_x_discrete(limits=orderpops)+
  scale_y_discrete(limits=orderpops)
gg
ggsave("heatmapTMRCAordered.pdf", useDingbats=FALSE)
...
library(ggridges) 
### Pairwise distances *********************************************
# now you can find some options to visualize which populations are closer to our target populations of the North Coast.
## Chiara's old plots
orderlables<-c("NORTH+NORTH", "SOUTH+SOUTH", "NORTH+SOUTH",  "NORTH+NORTH_A","SOUTH+SOUTH_A","NORTH_A+SOUTH_A")
g<-ggplot(meltpop2)
  g+ geom_boxplot(aes(infoCouple,averageDifference )) + coord_flip() +  scale_x_discrete(limits=rev(orderlables))
#  ggsave("BoxplotaverageDiff.pdf", useDingbats=FALSE)
#  
g+ geom_boxplot(aes(infoCouple,identical )) + coord_flip()+  scale_x_discrete(limits=rev(orderlables))
#  ggsave("BoxplotIdentical.pdf", useDingbats=FALSE)
#  
#   g+ geom_boxplot(aes(infoCouple,similar )) + coord_flip()+  scale_x_discrete(limits=rev(orderlables))
#   ggsave("BoxplotSimilar.pdf", useDingbats=FALSE)
#  
#   g+ geom_boxplot(aes(infoCouple,LESSsimilar )) + coord_flip()+  scale_x_discrete(limits=rev(orderlables))
#   ggsave("BoxplotLESSSimilar.pdf", useDingbats=FALSE)

## Lea's boxplots
g<-ggplot(meltpop2)+   #all north coast comparisons including outsiders as whole
...
g<-ggplot(filter(meltpop2, group1 == "Coast"| group2 == "Coast"))+   #only coast comparisons
...
g<-ggplot(meltpop2, na.rm = TRUE)+   #region comparisons
  geom_boxplot(aes(x = reorder(regionCouple, averageDifference, FUN = median), y = averageDifference))+
  coord_flip()+
  theme_pubr()
g
g<-ggplot(filter(meltpop2, pop1 != "Tlicho" & pop2 != "Tlicho"))+   #region comparisons without Tlicho
  geom_boxplot(aes(x = reorder(regionCouple, averageDifference, FUN = median), y = averageDifference))+
  coord_flip()+
  theme_pubr()
g
ggsave("boxAverageDiffRegionsWOTlicho.pdf", useDingbats = FALSE)

g<-ggplot(filter(meltpop2, !(pop1 %in% ancientpops) & !(pop2 %in% ancientpops)
                 & pop1 != "Tlicho" & pop2 != "Tlicho" & pop1 != "TobaSantaFe" & pop2 != "TobaSantaFe" &
                   pop1 != "Mocovi" & pop2 != "Mocovi"))+   #region comparisons without ancient and Tlicho, Mocovi and TobaSantaFe
  geom_boxplot(aes(x = reorder(regionCouple, averageDifference, FUN = median), y = averageDifference))+
  ...
  theme_pubr()
g
ggsave("boxAverageDifffinal.pdf", useDingbats = FALSE)
...
pdf("MapVariance.pdf",paper='special')
map(database = "world", regions = ".",xlim=c(-115,-35),ylim=c(-50,35), col="grey90", fill=TRUE,  lwd=0.1)
for(i in 1:nrow(resDiversityDF))  {
  points(x=resDiversityDF$lon[i], y=resDiversityDF$lat[i], pch=19,  cex=resDiversityDF$variance[i], # perhaps think about scaling them differently
         col=resDiversityDF$colour[i])
}
text(x=resDiversityDF$lon, y=resDiversityDF$lat, labels=resDiversityDF$code, pch=19,  cex=0.1, col="white")
dev.off()
...
# same for diversity but with something special for 1, don't know how though
gg + 
